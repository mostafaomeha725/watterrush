...
class OrderProgressTimelineWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final steps = [
      {'status': OrderStatus.pending, 'title': 'Pending', 'desc': 'We\'ve received your order'},
      {'status': OrderStatus.inProgress, 'title': 'Confirmed', 'desc': 'Your order is confirmed'}, // Mocking confirmed as inProgress for UI matching
      {'status': OrderStatus.inProgress, 'title': 'Preparing', 'desc': 'We are preparing your order'},
      {'status': OrderStatus.onTheWay, 'title': 'On The Way', 'desc': 'Your order is on the way'},
      {'status': OrderStatus.delivered, 'title': 'Delivered', 'desc': 'Order completed'},
    ];

    // For simplicity, find index. Pending=0, Confirmed=1, Preparing=2, OnTheWay=3, Delivered=4.
    int currentIndex = 0;
    if (currentStatus == OrderStatus.inProgress) currentIndex = 2; // Assuming preparing
